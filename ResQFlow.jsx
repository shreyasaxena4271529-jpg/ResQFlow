...
const BOT_RESPONSES = {
  flood: "🌊 **Flood Safety Protocol Activated**\n\nImmediate steps:\n• Move to higher ground NOW — avoid low-lying roads\n• Disable electricity at the main switch\n• Carry emergency kit: documents, water, medicines\n• Call NDRF: 011-24363260\n\nNearest shelter: Pragati Maidan Relief Camp (2.3km)",
  earthquake: "🌍 **Earthquake Response Guide**\n\n• DROP, COVER, HOLD — shelter under desk/table\n• Stay away from windows and exterior walls\n• After shaking stops: check for gas leaks, exit carefully\n• Emergency: 112 | NDMA Helpline: 1078\n\nSeismic monitoring active in your zone.",
  fire: "🔥 **Fire Emergency Response**\n\n• Evacuate immediately — do NOT use lifts\n• Crawl low if there is smoke\n• Close doors behind you to slow fire spread\n• Call Fire Department: 101\n\nAI detection has flagged your area — responders dispatched.",
  default: "I'm ResQFlow AI, your multilingual disaster assistant. I can help with:\n\n• **Real-time alerts** for floods, earthquakes, fires, cyclones\n• **Evacuation routes** and nearest shelter locations\n• **Emergency contacts** for your region\n• **First aid guidance** and medical advice\n\nAsk me anything — I respond in English, Hindi, Tamil, Bengali, Telugu, and more. 🇮🇳",
};

function getLang(msg) {
  if (/flood|बाढ़|홍수/i.test(msg)) return "flood";
  if (/quake|earth|भूकंप/i.test(msg)) return "earthquake";
  if (/fire|आग|wildfire/i.test(msg)) return "fire";
  return "default";
}
...
export default function ResQFlow() {
  ...
  const sendMessage = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      const key = getLang(userMsg.text);
      setMessages(m => [...m, { role: "bot", text: BOT_RESPONSES[key], time: "just now" }]);
      setLoading(false);
    }, 1200);
  };
              ...
              <div className="card" style={{ height: "calc(100vh - 120px)", display: "flex", flexDirection: "column", padding: 0 }}>
                <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--dim)" }}>Powered by Gemini API · Text · Audio · Vision</div>
                  </div>
